...
Branch API follows [GitHub's branch documentation](https://docs.github.com/en/rest/branches/branches#list-branches). Live app authentication, repository reads, graph construction and Actions reads have been verified. Browser user authorization and webhook delivery still require live verification.
...
## Backend hardening (September 12)

Repository indexing uses deterministic path order and recovers truncated recursive GitHub trees through individual subtrees (up to 200 tree requests). Source limits are 500 files, 100 KB per file, and 4 MB total; actual decoded sizes are checked. The combined serialized snapshot and graph must remain under 8 MB before persistence. Coverage remains partial: this is static JS/TS import analysis, not complete runtime understanding.

Authenticated project APIs share MongoDB-backed budgets: 120 requests per minute and 60 mutation requests per hour per user, across processes. Cancellation and refresh are exempt from the hourly mutation budget so recovery remains available; the minute budget still applies. Repeated and invalid requests count. Existing AI requests retain their separate 10/hour budget. Responses use 429 with Retry-After. These are MVP abuse controls, not billing quotas or storage retention policies.

Set OPERATIONS_TOKEN to a random secret of at least 32 characters. GET /internal/metrics requires that bearer token and reports queue counts, expired leases, age of the oldest ready job, failed jobs, uncertain dispatches, and process uptime. No repository content or credentials are returned. Poll from your monitoring system; alert on expired leases, increasing ready-job age, and new uncertain dispatches. This endpoint exposes metrics but does not provision an external alerting service.

Audit: exhausted interrupted jobs now settle as failed from the normal polling function, without depending on a separate cleanup loop. Disconnect checks, job idempotency, cross-account access, callback cancellation, webhook deduplication, and dispatch uncertainty have regression coverage. Live GitHub App user OAuth and public webhook delivery remain unverified. Snapshot persistence now uses a transaction that writes the project lease and job cancellation fence together with the revision. MongoDB must be a replica set or sharded cluster; standalone MongoDB cannot save analyses. MongoDB replication/backups, storage retention and deployment monitoring configuration remain operational work.

Live verification: the backend GitHub client dispatched and polled [Actions run 34673065276](https://github.com/Nmilien34/velament/actions/runs/34673065276), which completed successfully at commit 82bf95d585078efaf565cfb83b59b9af4ac38d0e. This tested the currently pushed CI revision, not the uncommitted changes. A repository-scoped GitHub App installation token was used and revoked afterward. The full signed-in dispatch service stopped before submission because the CLI OAuth token cannot access the App-user installation endpoint (403); that path requires a GitHub App user OAuth connection. No product authorization checks were bypassed or relaxed.
