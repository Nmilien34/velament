...
Custom runtime conditions, export arrays, general semver ranges, pnpm YAML workspace declarations, external packages and missing build outputs remain unresolved. No `dist`-to-`src` mapping is inferred. This is static source association, not proof of the installed dependency graph or runtime behavior. Package manifests share the existing capture budgets; existing revisions need reanalysis to include them.
