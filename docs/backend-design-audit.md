...
| Don't break this / pinned features | Pin baselines and explicit source/edge comparison exist | Automatic per-push pin assessment and feature-targeted regression execution are not implemented |
...
2. Add automatic pin comparison when a new snapshot completes, with deduplicated activity and explicit changed/unknown labels. Do not equate unchanged source with a passing feature test.
