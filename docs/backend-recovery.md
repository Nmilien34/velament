...
## User-uploaded test reports

`PUT /api/projects/:projectId/assessments/:id/test-report` accepts the shared `testReportInput` contract: `runId`, `attempt`, exact `sha`, `environment`, `mockedBoundaries`, and 1–100 uniquely named `tests` with `passed`, `failed`, or `skipped` outcomes. The run must be completed and match the project, commit and current attempt. A transaction fences the run check against concurrent imports. PUT explicitly replaces the previous report on this assessment; no report history is retained.

Assessment reads expose `reportEvidence` with the report and staleness. Changed/archived features, missing runs and newer attempts make the report stale. Any failure yields reported outcome `failed`; all passes yield `passed`; otherwise `incomplete`. These are user-reported outcomes, not verified feature behavior: `featureVerification` remains `not-established`. Environment and mock boundaries are user declarations. Reports are embedded in assessments and purged with them. Automatic CI artifact ingestion and independent test-to-requirement verification remain unimplemented.

`DELETE /api/projects/:projectId/assessments/:id/test-report` clears the uploaded report and returns 204 (also if already absent). The assessment and separately associated run are preserved. Missing or foreign assessments return 404 under project access controls.

`GET /api/projects/:projectId/runs/:id/artifacts?page=1` lists GitHub artifact metadata for an imported run using repository authorization. Pages contain at most 100 artifacts, up to page 100, with `nextPage` and `truncated`. Expired artifacts remain visible. Download URLs are not exposed or followed. Artifact discovery does not verify the run attempt or report contents; downloading and report ingestion remain separate work. See https://docs.github.com/en/rest/actions/artifacts#list-workflow-run-artifacts.

## Import a GitHub report artifact

`POST /api/projects/:projectId/assessments/:id/import-report` accepts `{ "runId": "<imported run ID>", "artifactId": 123 }`. The selected artifact must belong to the run and commit, be unexpired and at most 2 MB. GitHub's download redirect is followed without credentials, only to supported HTTPS Azure blob / Actions storage hosts. Other storage hosts fail closed. The ZIP is read in memory, never extracted to disk, with at most 100 entries and a 256 KB limit for one root `velament-test-report.json`.

The JSON follows the upload contract, except the backend supplies `runId`. Its declared attempt must match the current imported run. The stored provenance is `github-artifact` with `artifactId`; this confirms the retrieval path, not that tests correctly cover a requirement. GitHub artifact metadata does not independently establish the report's attempt assertion. Revocation/project lifecycle checks fence persistence, as does the completed-run/attempt check. This is an explicit import, not unattended webhook ingestion. Live artifact download-to-save verification remains outstanding.

New snapshots capture `tsconfig.json`/`jsconfig.json` within existing repository limits. Static graph resolution supports exact and single-wildcard `compilerOptions.paths` mappings, relative to `baseUrl` or the config directory. The nearest directory's config is used (TypeScript preferred over JavaScript). Missing targets, invalid configs and inherited configurations remain unresolved. Config `include`/`exclude`, project references, package exports and runtime bundler resolution are not modeled; edges are static hypotheses. Existing snapshots require reanalysis to capture configuration files.

Relative config inheritance is supported when the parent config is captured. Named `tsconfig.*.json`/`jsconfig.*.json` files are now eligible for capture. Inherited path mappings retain their declaring directory when no base URL is present; an explicit child base URL overrides the inherited base URL. Missing parents, package-based inheritance, arrays of parents, cycles, and chains beyond 10 configs are left unresolved. New analysis is required to capture additional configs.
