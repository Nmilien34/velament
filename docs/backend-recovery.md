...
`complete` means every eligible batch completed, not full repository coverage or runtime verification. Files larger than one batch are explicitly skipped; snapshot exclusions remain visible. Candidates are kept in their originating batches, without speculative cross-batch merging. Stop submitting batches to pause; there is no unattended background continuation or cancellation of an already sent provider request. Access revocation and project lifecycle changes fence the result write.
